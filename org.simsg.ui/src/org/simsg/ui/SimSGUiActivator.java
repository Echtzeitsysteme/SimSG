...
public class SimSGUiActivator extends AbstractUIPlugin implements BundleActivator {

	private static Logger logger = Logger.getLogger(SimSGUiActivator.class);
	
	// The shared instance
	private static SimSGUiActivator plugin;
	 ...
	 * @return
	 */
	public static SimSGUiActivator getDefault() {
		if (plugin == null)
			throw new IllegalStateException(
					String.format("Singleton of class %s not initialized yet.", SimSGUiActivator.class.getName()));
	...
	public static boolean configureLogging(final URL configFile) {
		try {
			...
			Logger.getRootLogger().addAppender(new SimSGConsole(configFile));

			// Indicate success
			root.info("Logging to SimSG console. Configuration: " + configurationStatus);
			return true;
		} catch (Exception e) {
